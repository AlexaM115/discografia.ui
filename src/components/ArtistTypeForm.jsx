...
const ArtistTypeForm = ({ item, onSuccess, onCancel }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            let savedItem;
            if (item) {
                savedItem = await artistTypeService.update(item.id, formData);
                if (!savedItem) {
                    savedItem = { ...item, ...formData };
                }
                onSuccess(savedItem, true);
            } else {
                savedItem = await artistTypeService.create(formData);
                if (!savedItem) {
                    savedItem = { 
                        id: Date.now(),
                        ...formData 
            ...
            setError(error.message || 'Error al guardar el tipo de artista');
        } finally {
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
                <div className="p-6">
                    <h2 className="text-xl font-bold text-gray-800 mb-4">
                        {item ? 'Editar Tipo de Artista' : 'Nuevo Tipo de Artista'}
                    </h2>
